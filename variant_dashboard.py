...
def load_variant_data():
    """Load from variant_analysis.xlsx — generated by variant_export.py"""
    # Try local first
    ...
    if os.path.exists(local):
        try:
            ...
            sum_df   = pd.read_excel(local, sheet_name="Variant Summary",  engine="openpyxl",
                                     header=3)
            return size_df, color_df, None
        except Exception as e:
            return None, None, str(e)
    return None, None, "variant_analysis.xlsx not found. Run variant_export.py first."

@st.cache_data(ttl=300)
def load_main_data():
    """Load main products Excel for brand/category filter"""
    base = r"C:\Users\Legion\Desktop\odoo_export"
...
def main():
    ...
    if err or size_df is None:
        st.error(f"Could not load variant data: {err}")
        st.info("""
        **To generate variant data:**
        1. Download `variant_export.py` from Claude
        2. Put it in your `odoo_export` folder
        3. Run: `python variant_export.py --brand SALT`
        4. Refresh this page
        """)
        st.stop()
    ...
    if "Units Sold" in size_df.columns and "In Stock" in size_df.columns:
        ...
        size_status_counts  = size_df["Status"].value_counts() if "Status" in size_df.columns else {}
        color_status_counts = color_df["Status"].value_counts() if "Status" in color_df.columns else {}
    ...
    if view == "Size Performance":
        ...
        if "Size" in sf.columns and "Units Sold" in sf.columns:
            ...
            best_str_size = size_agg.loc[size_agg["STR_%"].idxmax(), "Size"] if len(size_agg) > 0 else "N/A"
            st.markdown(
            ...
            with col1:
                ...
                chart_data = chart_data.reindex(ordered + others).dropna()
                st.bar_chart(chart_data)

            with col2:
                st.markdown("**STR % by Size**")
                str_data = size_agg.set_index("Size")["STR_%"]
                ordered_str = [s for s in SIZE_ORDER if s in str_data.index]
                others_str  = [s for s in str_data.index if s not in SIZE_ORDER]
                str_data = str_data.reindex(ordered_str + others_str).dropna()
                st.bar_chart(str_data)
        ...
        if "Color" in cf.columns and "Units Sold" in cf.columns:
            ...
            with col2:
                ...
                top_str = color_agg.nlargest(20,"STR_%")
                st.bar_chart(top_str.set_index("Color")["STR_%"])
        ...
        st.markdown('<div class="sec-title">📊 Size × Color Performance Matrix</div>',
                    unsafe_allow_html=True)
        st.markdown("Shows total units sold per Size/Color combination across all products")

        if "Size" in size_df.columns and "Color" in color_df.columns:
            # We need the raw variant data for a proper cross-tab
            # For now show top performers from each
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("**Top Sizes (by revenue)**")
                if "Size" in sf.columns:
                    ...
                    st.dataframe(sa.reset_index().rename(columns={"Units Sold":"Units Sold"}),
                                 use_container_width=True, hide_index=True)
            with col2:
                st.markdown("**Top Colors (by revenue)**")
        ...
        with col1:
            ...
            if "Status" in size_df.columns:
                ...
                if len(top_sf) > 0:
                    st.dataframe(top_sf[["Product Name","Size","Units Sold","In Stock","STR %","Status"]],
                                 use_container_width=True, hide_index=True)
                else:
        ...
        with col2:
            ...
            if "Status" in color_df.columns:
                top_cf = color_df[color_df["Status"]=="Super Fast"].sort_values(
                    "Units Sold", ascending=False).head(20)
                if len(top_cf) > 0:
                    st.dataframe(top_cf[["Product Name","Color","Units Sold","In Stock","STR %","Status"]],
                                 use_container_width=True, hide_index=True)
                else:
        ...
        with col3:
            ...
            if "Status" in size_df.columns:
                ...
                if len(dead_s) > 0:
                    st.dataframe(dead_s[["Product Name","Size","Units Sold","In Stock","STR %"]],
                                 use_container_width=True, hide_index=True)
        ...
        with col4:
            ...
            if "Status" in color_df.columns:
                dead_c = color_df[color_df["Status"]=="Dead"].sort_values(
                    "In Stock", ascending=False).head(20)
                if len(dead_c) > 0:
                    st.dataframe(dead_c[["Product Name","Color","Units Sold","In Stock","STR %"]],
                                 use_container_width=True, hide_index=True)
