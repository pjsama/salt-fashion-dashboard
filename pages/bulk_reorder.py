...
# ── Pre-compute size / color / store data (used by popup AND flat tables) ─────
sz = pd.DataFrame()
...
# ── Trigger popup dialog when a product is selected ───────────────────────────
if sel_popup != "— select a product to inspect —":
    prow_sel = prod_sum[prod_sum["Product Name"] == sel_popup].iloc[0]
    show_product_dialog(sel_popup, prow_sel, sz if not sz.empty else None,
                        cl if not cl.empty else None, _ps_all, target_weeks, fmt_npr)
...
if sz.empty:
    ...
    st.caption(f"{len(sz):,} size rows · 🔵 Order (Wk) = {target_weeks}-week buffer per size · 🟣 Order (STR) = restore level")
