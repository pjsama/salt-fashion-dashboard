...
def clean_df(df):
    ...
    # Normalise STR Status column
    str_col = next((c for c in df.columns if "str status" in c.lower() or
                    c.lower() == "velocity"), None)
    if str_col and str_col != "STR Status":
        df = df.rename(columns={str_col: "STR Status"})
